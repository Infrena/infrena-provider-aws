...
func TestVPCFlagsSpellingsAndTags(t *testing.T) {
	...
	// Tags carry NO flag. They are user data, not a value the cloud picks, and the host keys both of the
	// behaviours that matter off Computed: its configuration generator omits a computed attribute unless it is
	// also ForceNew, and its diff skips an unconfigured attribute when it is computed. While the tag map was
	// optional+computed it landed in the same bucket as EnableDnsSupport, so `import --generate` wrote no tags
	// for 905 of the 958 taggable types — and then adding one tag by hand diffed one against the three in state
	// and proposed deleting the two the user had never seen. Optional alone is not the fix and is refused
	// outright by infrena (pkg/schema: "Optional exists to pair with Computed").
	if tags := attr(t, vpc, "Tags"); tags.Computed || tags.Optional || tags.Required || tags.ForceNew {
		t.Errorf("Tags = %+v, want no flags at all: computed tags are dropped from generated configuration", tags)
	}
