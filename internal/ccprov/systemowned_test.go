...
func TestTheFlagIsSetOnlyOnEvidence(t *testing.T) {
	...
	for _, c := range []struct {
		...
		// An instance an Auto Scaling group, an EC2 Fleet or a Spot Fleet launched. Adopting one is the mistake
		// that looks like a bug days later: the controller replaces it on its own schedule — minutes, for a spot
		// fleet draining Batch jobs — and every plan after that proposes recreating an instance ID that no longer
		// exists anywhere. infrena cannot manage a lifecycle it does not control, so it must not claim to.
		{"an instance an Auto Scaling group launched", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-1", "Tags": tagList("aws:autoscaling:groupName", "web-asg")}, blind,
			"tagged aws:autoscaling:groupName"},
		{"an instance an EC2 Fleet launched", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-2", "Tags": tagList("aws:ec2:fleet-id", "fleet-abc")}, blind,
			"tagged aws:ec2:fleet-id"},
		{"an instance a Spot Fleet request launched", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-3", "Tags": tagList("aws:ec2spot:fleet-request-id", "sfr-abc")}, blind,
			"tagged aws:ec2spot:fleet-request-id"},

		// The other half of the rule, and the half worth guarding. A user's own instance is theirs to manage
		// however it was launched, and a tag they typed is not AWS's claim on it. Flagging one drops it from
		// `import` silently, which is the failure this whole file exists to avoid.
		{"a spot instance a user launched themselves", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-4", "InstanceLifecycle": "spot"}, blind, ""},
		{"an instance tagged by hand to look managed", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-5", "Tags": tagList("autoscaling:groupName", "web-asg")}, blind, ""},
		{"an instance with a launch template but no controller", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-6", "Tags": tagList("aws:ec2launchtemplate:id", "lt-abc")}, blind, ""},
		{"an ordinary instance", "aws.ec2.instance",
			map[string]any{"InstanceId": "i-7", "Tags": tagList("Name", "bastion")}, blind, ""},
