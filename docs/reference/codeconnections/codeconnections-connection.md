...
**Import ID:** `<region>/ConnectionArn` (AWS::CodeConnections::Connection)
...
| `HostArn` | host_arn | `string` | optional, computed, provider-chosen, replaces on change | aws.codeconnections.host.HostArn | The host arn configured to represent the infrastructure where your third-party provider is installed. You must specify either a ProviderType or a HostArn. |
...
| `Tags` |  | `map` | tags map |  | Specifies the tags applied to a connection. |
