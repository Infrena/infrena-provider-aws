# aws.modelexplainabilityjobdefinition
...
**Import ID:** `<region>/JobDefinitionArn` (AWS::SageMaker::ModelExplainabilityJobDefinition)
...
| `ModelExplainabilityAppSpecification` | model_explainability_app_specification | `map` | required, replaces on change |  | Container image configuration object for the monitoring job. |
| `ModelExplainabilityBaselineConfig` | model_explainability_baseline_config | `map` | optional, computed, provider-chosen, replaces on change |  | Baseline configuration used to validate that the data conforms to the specified constraints and statistics. |
| `ModelExplainabilityJobInput` | model_explainability_job_input | `map` | required, replaces on change |  | The inputs for a monitoring job. |
| `ModelExplainabilityJobOutputConfig` | model_explainability_job_output_config | `map` | required, replaces on change |  | The output configuration for monitoring jobs. |
