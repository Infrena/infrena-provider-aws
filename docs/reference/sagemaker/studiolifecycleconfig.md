...
| `Tags` |  | `map` | optional, computed, provider-chosen, replaces on change, tags map |  | Tags to be associated with the Lifecycle Configuration. Each tag consists of a key and an optional value. Tag keys must be unique per resource. Tags are searchable using the Search API. |
