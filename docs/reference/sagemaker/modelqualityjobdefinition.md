...
**Import ID:** `<region>/JobDefinitionArn` (AWS::SageMaker::ModelQualityJobDefinition)
...
| `ModelQualityAppSpecification` | model_quality_app_specification | `map` | required, replaces on change |  | Container image configuration object for the monitoring job. |
| `ModelQualityBaselineConfig` | model_quality_baseline_config | `map` | optional, computed, provider-chosen, replaces on change |  | Baseline configuration used to validate that the data conforms to the specified constraints and statistics. |
| `ModelQualityJobInput` | model_quality_job_input | `map` | required, replaces on change |  | The inputs for a monitoring job. |
| `ModelQualityJobOutputConfig` | model_quality_job_output_config | `map` | required, replaces on change |  | The output configuration for monitoring jobs. |
