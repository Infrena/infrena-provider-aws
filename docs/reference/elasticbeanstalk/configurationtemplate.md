...
| `SourceConfiguration` | source_configuration | `map` | optional, computed, provider-chosen, replaces on change, write-only |  | An Elastic Beanstalk configuration template to base this one on. If specified, Elastic Beanstalk uses the configuration values from the specified configuration template to create a new configuration. |
