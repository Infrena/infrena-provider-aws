...
**Import ID:** `<region>/Arn` (AWS::Chatbot::SlackChannelConfiguration)
...
| `SlackChannelId` | slack_channel_id | `string` | required |  | The id of the Slack channel |
| `SlackWorkspaceId` | slack_workspace_id | `string` | required, replaces on change |  | The id of the Slack workspace |
...
| `Tags` |  | `map` | optional, computed, provider-chosen, tags map |  | The tags to add to the configuration |
