...
func project(t *testing.T, body string) *env {
	...
	e.fake.Register(sg)
	writeFile(t, filepath.Join(e.dir, "infrena.yml"), body)
	return e
}

func fixture(t *testing.T, name string) string {
	t.Helper()
	data, err := os.ReadFile(filepath.Join("testdata", name, "infrena.yml"))
	if err != nil {
		t.Fatal(err)
	}
	return string(data)
...
func (e *env) editConfig(t *testing.T, from, to string) {
	t.Helper()
	path := filepath.Join(e.dir, "infrena.yml")
	body, _ := os.ReadFile(path)
	if !strings.Contains(string(body), from) {
		t.Fatalf("infrena.yml does not contain %q", from)
	}
...
// TestTheWorkflow walks the whole explain, plan, apply and drift checklist against a VPC, a
// subnet and a security group.
func TestTheWorkflow(t *testing.T) {
	...
	t.Run("a value AWS chose that changes is not a diff", func(t *testing.T) {
		...
		if ops := e.planOps(t); len(ops) != 0 {
			t.Fatalf("an unset provider-chosen attribute plans %v, want nothing", ops)
		}
	})
	...
	t.Run("discover names resources from their Name tag, and import --generate references between them", func(t *testing.T) {
		...
		// The tags the resources carry must reach the generated configuration, not only the state file. Nothing
		// asserted this until 2026-09-18, and the gap shipped a real bug: while the catalog marked the tag map
		// optional+computed, infrena's generator read it as "a setting with a cloud default" and omitted it for
		// 905 of the 958 taggable types. Tags still reached state, so a plan straight after import was clean and
		// the check below passed — which is precisely why that check alone was not enough. The damage appeared a
		// step later: adding one tag by hand diffed one tag against the three in state and proposed deleting the
		// two the user had never seen. Both names below are already in this subtest's fixture.
		// Assert on the Tags BLOCK, not on the tag values. Discovery names these resources FROM their Name tag
		// (vpc-app1, subnet-app1a), so the strings "app1" and "app1a" appear in the generated file either way —
		// a first version of this check looked for them and passed with the bug fully restored, which made it
		// one more test that could not fail. The key infrena emits is the attribute's Display name, and the tag
		// map has no alias, so it writes `Tags:` with the pairs nested under it.
		for _, want := range []string{"Tags:", "Name: app1a", "Name: app1"} {
			if !strings.Contains(generated, want) {
				t.Fatalf("generated configuration has no %q; tags reach state but not the file, so adding one tag "+
					"by hand would diff against the rest and propose deleting them:\n%s", want, generated)
			}
		}
...
// TestTheInfrenaUnderTestSpeaksTheManifestsProtocol applies the manifest's compatibility rules to the
