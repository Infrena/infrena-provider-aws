...
**Under construction on branch `first-slice`: Tasks 1–4 of 12 done** (schemas, binary, module pinned at infrata `v0.2.0`; instance configuration and credentials; `internal/ec2fake`, the fake EC2/STS endpoint; provider IDs and error classification). The design and the first vertical slice (`aws.vpc` + `aws.subnet`) are in
