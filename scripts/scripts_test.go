...
func TestReleaseCheckRefusesABinaryThatDoesNotKnowItsVersion(t *testing.T) {
	v := manifestVersion(t)
	out, err := run(t,
		[]string{"PLUGIN_VERSION_SYMBOL=github.com/infrena/infrena-provider-aws/internal/awsprov.NoSuchVariable"},
		"release-check", "v"+v)
	if err == nil {
...
func TestBuildReleaseNamesArchivesByTheInstallConvention(t *testing.T) {
	...
	stem := "infrena-plugin-aws_" + v + "_linux_amd64"
	for _, name := range []string{stem + ".tar.gz", "infrena-plugin-aws_" + v + "_windows_amd64.zip"} {
	...
	// README.md is Task 17's; the archive carries it once it exists.
	want := []string{stem + "/infrena-plugin-aws", stem + "/plugin.yaml"}
